...
- [ ] **AC-7 (N1 + N2 rationale).** N1 = 90 seconds. Rationale: the Planner's `dev-assist-work-queue-poll` runs at 60-second cadence per `MULTI-HERMES-CONTRACT.md` § 8.1 step 4; N1 = 60 + 30s slack covers one full poll cycle plus cronjob jitter (per `MULTI-HERMES-CONTRACT.md` § 9.1 `StartLimitBurst=5` over `StartLimitIntervalSec=300` envelope). N2 = 300 seconds. Rationale: N1 + typical Planner LLM call latency upper bound (≤ 180s at p95 for `qwen3p6-plus` per `MODEL-CATALOG.md` v0.3.0 § 4.3 latency table — Executor verifies the p95 number at branch-cut; if the table has shifted, the Executor files Q-TKT and adjusts N2) + result write-back + observability flush (≤ 5s per `OBSERVABILITY-CONTRACT.md` § 9 batch-write semantics) + buffer. If N1 or N2 needs to be re-derived because `MULTI-HERMES-CONTRACT.md` § 8.1 cadence or `MODEL-CATALOG.md` v0.3.0 § 4.3 p95 changes, the Executor files Q-TKT-041-NN; do NOT silently widen the smoke timeouts.
...
- Include the Reviewer artifact path (`docs/reviews/RV-CODE-NNN.md`) and verdict before merge-safe sign-off. Cross-family Reviewer-LLM (Kimi K2.6 Moonshot on opencode + OmniRoute) is MANDATORY per the AUDIT-001 / TKT-040 precedent.
...
- **N1 = 90s and N2 = 300s are derived from current cadence and `MODEL-CATALOG.md` v0.3.0 § 4.3 p95 numbers.** If either changes, N1/N2 must be re-derived. Mitigation: AC-7 mandates a Q-TKT rather than a silent widening; the smoke fails loudly rather than silently passing on a stretched envelope.
...
- AUDIT-002 (TKT-034 v0.3.1) MUST be merged on `main`. AUDIT-003 extends the interactive installer with the `--smoke-mode` flag; the installer surface is owned by AUDIT-002. Status: merged via PR #169 (2026-05-11). Confirmed on `main` at HEAD `3e298c2`.
