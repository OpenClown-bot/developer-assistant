...
**`--remotes` (all remote-tracking branches, simulating a fresh clone):**

```
Devin AI <158243242+devin-ai-integration[bot]@users.noreply.github.com>
devin-ai-integration[bot] <158243242+devin-ai-integration[bot]@users.noreply.github.com>
errydayiwakingup <JuaresPresswood75@hotmail.com>
lindwurm.22.shane <yourmomsenpai@yandex.ru>
ohemaeeee232 <ohemaeeee232@pinmx.net>
OpenClown-bot <bot@openclown-bot.dev>
OpenClown-bot <yourmomsenpai@yandex.ru>
OpenClown-lover <yourmomsenpai@yandex.ru>
polfkfn1s <polfkfn1s@pinmx.net>
Strategic Orchestrator <strategic-orchestrator@developer-assistant.local>
```

**Unexpected identities (PII) found:**
- `errydayiwakingup <JuaresPresswood75@hotmail.com>`
- `lindwurm.22.shane <yourmomsenpai@yandex.ru>`
- `ohemaeeee232 <ohemaeeee232@pinmx.net>`
- `OpenClown-bot <yourmomsenpai@yandex.ru>`
...
```
git log --all --format='%B' | Select-String -Pattern '<PII-blocklist>' | Measure-Object
git log --all --format='%B' | Select-String -Pattern 'devin-[a-f0-9]{8,}' | Measure-Object
```
...
- `git log --remotes --format='%aN <%aE>' | sort -u` → 10 identities, 5 of which are unexpected PII (Finding 1 evidence).
- `git log origin/rv/arch-001-stale-refs --format='%B' | Select-String -Pattern '<PII-blocklist>'` → 26 matches (Finding 2 evidence).
- `git log origin/rv/arch-001-stale-refs --format='%B' | Select-String -Pattern 'devin-[a-f0-9]{8,}'` → 7 matches (Finding 2 evidence).
