...
- **Routing-layer details are owned by `ADR-011-routing-layer.md`** (PR-E): this ADR records WHAT models are picked per role and WHY the catalog shape was chosen; ADR-011 records WHERE the calls go (OmniRoute on `127.0.0.1:20128` primary, with Fireworks as configured backend; OpenRouter direct as backup) and HOW the binding precondition (OmniRoute supports the Fireworks backend) is verified. Catalog changes that touch routing topology (e.g., switching the configured backend, adding a third backup routing layer) are governed by ADR-011, not this ADR; both ADRs must agree before such a change merges. The reciprocal cross-reference lives in `ADR-011-routing-layer.md` Consequences.
