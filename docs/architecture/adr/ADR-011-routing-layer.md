...
- **Per-role model assignment and fallback chains are owned by `ADR-009-model-assignment-and-fallback.md` v0.1.1**: this ADR records WHERE calls go (OmniRoute primary, Fireworks-as-backend, OpenRouter backup) and HOW the OmniRoute-supports-Fireworks binding precondition is verified at install/upgrade. ADR-009 records WHAT model each role uses (`MODEL-CATALOG.md` v0.2.0 § 4.1: orchestrator → minimax-m2.7, planner → qwen3.6-plus, architect → deepseek-v4-pro, executor → glm-5.1, reviewer → kimi-k2.6) and the per-role fallback chain ordering. Catalog content changes that do not touch routing topology (e.g., adding a sixth Fireworks-hosted model to the catalog, reordering a fallback chain within Fireworks) are governed by ADR-009 alone; routing-topology changes (e.g., switching the configured backend behind OmniRoute, adding a third backup routing layer, allowing direct provider SDKs) require an update to this ADR. Both ADRs must agree before a change touching either dimension merges. The reciprocal cross-reference lives in `ADR-009-model-assignment-and-fallback.md` Consequences.
- **OmniRoute server-side middleware extension API verification (M-001 secondary path; record per `ARCH-001.md` v0.3.0 § 23 and TKT-031 § 10 Execution Log)**: at the time this ADR was drafted (2026-05-06), the OmniRoute v3.7.x extension API for server-side request-completion middleware has NOT been verified by reading OmniRoute source. The client-side LLM instrumentation in TKT-031 (`llm_client_instrumentation.py`) is therefore the SINGLE SOURCE OF TRUTH for `llm_calls` rows in v0.1. If the middleware API is later verified by an Executor reading OmniRoute source, the verified version/commit SHA is recorded in this Consequences list and TKT-031's `omniroute_middleware.py` is implemented as a parallel cross-check; otherwise that module is omitted from v0.1.
