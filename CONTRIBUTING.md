...
| --- | --- | --- | --- | --- |
| Strategic Orchestrator | MiniMax M2.7 (main) / Kimi K2.6 / Qwen 3.6 Plus / DeepSeek V4 Pro | opencode (Founder's Windows PC) | Session-level conductor: ticket selection, TO delegation, ratification audit pass-2, merge-safe sign-off, founder-facing teaching | `docs/session-log/`, `docs/meta/`, `docs/orchestration/`, `docs/backlog/` (light edits / new entries), ticket frontmatter promotions (`status`, `arch_ref`, `version`, `updated`) + `§10 Execution Log` append-only fills, `docs/questions/` (light edits / new questions), `.github/workflows/`, `.pr_agent.toml`, `CONTRIBUTING.md`, `AGENTS.md`, `README.md`, `docs/prompts/` (when adapting role prompts) |
| Ticket Orchestrator | MiniMax M2.7 (main) / Kimi K2.6 / Qwen 3.6 Plus / DeepSeek V4 Pro | opencode (Founder's Windows PC) | Per-TKT cycle runner: drafts Executor + Reviewer NUDGE files, runs cross-reviewer audit pass-1, hands back to Strategic Orchestrator | Per-ticket clerical sub-PRs scoped to one TKT, frontmatter promotion of own TKT, BACKLOG entries scoped to own TKT, NUDGE files (Founder-pasted, generally not committed) |
...
| Business Planner | Qwen 3.6 Plus (main) / Kimi K2.6 / MiniMax M2.7 / DeepSeek V4 Pro | ChatGPT Plus (web) | Clarifies product intent and MVP scope | `docs/prd/` |
| Architect | DeepSeek V4 Pro (main) / Kimi K2.6 / GLM 5.1 / Qwen 3.6 Plus | Codex CLI / opencode CLI / Windsurf | Defines system architecture, ADRs, and implementation tickets | `docs/architecture/`, `docs/tickets/` |
| Code Executor | GLM 5.1 (main) / DeepSeek V4 Pro / Kimi K2.6 / Qwen 3.6 Plus | opencode + OmniRoute | Implements one approved ticket at a time | `src/`, `tests/`, explicitly allowed config files, and ticket Section 10 Execution Log only |
| Reviewer | Kimi K2.6 (main) / DeepSeek V4 Pro / GLM 5.1 / Qwen 3.6 Plus | opencode + OmniRoute | Reviews PRs against ticket, architecture, ADRs, and CI | `docs/reviews/` |
...
- **Strategic Orchestrator** is the dev-time session-level conductor (MiniMax M2.7 main in an opencode tab on the Founder's PC; Kimi K2.6 / Qwen 3.6 Plus / DeepSeek V4 Pro fallback via opencode + OmniRoute). It is the partner the Ticket Orchestrator hands back to.
- **Ticket Orchestrator** is the dev-time per-TKT cycle runner (MiniMax M2.7 main in a separate opencode tab; Kimi K2.6 / Qwen 3.6 Plus / DeepSeek V4 Pro fallback via opencode + OmniRoute). One TO session per TKT, never reused.
